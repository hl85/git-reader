...
/// 纯函数无副作用，便于单元测试。
enum MarkdownBlockClassifier {

    /// 分类顶层块元素
    static func classify(_ markup: Markup) -> BlockElement {
        switch markup {
    ...
    static func listDepth(_ markup: Markup) -> Int {
        var depth = 0
    ...
    /// 从 Table 提取表头和行数据
    static func extractTable(_ table: Table) -> TableData {
