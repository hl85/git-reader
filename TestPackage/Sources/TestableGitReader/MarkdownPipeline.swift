...
public struct MarkdownPipeline {
    ...
    public struct Result {
        ...
        public init(frontmatter: [String: Any], document: Markdown.Document) {
    ...
    public static func process(fileAt fileURL: URL) -> Result? {
    ...
    /// 处理原始文本
    public static func process(rawText: String, fileURL: URL) -> Result? {
        // 1. 分离 Frontmatter
    ...
    public static func processBody(rawText: String, fileURL: URL) -> Document {
        let noteContent = FrontmatterSplitter.split(rawText)
        let directory = fileURL.deletingLastPathComponent()
