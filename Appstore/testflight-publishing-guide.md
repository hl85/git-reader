...
3. 填写基本信息：
   ...
   * **套装 ID (Bundle ID)**：选择您在 Xcode 中配置的 Bundle ID（例如 `com.yourname.GitReader`）。
   * **SKU**：一个独特的标识符，可以用 `com.yourname.GitReader`。
   * **用户访问权限**：完全访问权限。
4. 点击 **创建**。
...
1. 在 Xcode 中打开 `GitReader.xcodeproj`。
2. 选择项目根节点 -> 选择 **GitReader** Target -> 进入 **Signing & Capabilities** 标签页。
