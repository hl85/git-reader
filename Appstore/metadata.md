...
* **副标题 (Subtitle)**: (限制 30 字符以内，突出核心定位)
  * **推荐方案**：`Obsidian 笔记 Git 极速阅读器` (20 字符) —— 目标受众极其精准，直击痛点。
  * **备选方案 1**：`只读无冲突的 Git 笔记阅读器` (21 字符) —— 强调“只读、无冲突”的安全属性。
  * **备选方案 2**：`沉浸式 Obsidian 笔记阅读器` (20 字符) —— 强调 Claude 风格的沉浸式美学。
...
* **宣传文本 (Promotional Text)**: (限制 170 字符，显示在 App Store 描述顶部，可随时修改无需审核)
  `专为 Obsidian + Git 用户打造的 iOS 极速只读笔记阅读器。零冲突风险，一键同步，完美渲染 WikiLinks 与本地图片，带给您 Claude 般温润沉浸的阅读体验。` (80 字符)
...
```text
Gits Reader —— 专为 Obsidian + Git 用户打造的 iOS 移动端“纯净只读”看板与阅读器。
